...
function QtLoader(config)
{
    ...
    function loadEmscriptenModule(applicationName, contenUrl, theme, forground, background, primary, accent) {
        ...
        var emscriptenModuleSourcePromise = fetchText("assets/plugins/wasm/" + applicationName + ".js").then(function(source) {
            emscriptenModuleSource = source
        });

        // Fetch and compile wasm module
        var wasmModule = undefined;
        var wasmModulePromise = fetchCompileWasm("assets/plugins/wasm/" + applicationName + ".wasm").then(function (module) {
            wasmModule = module;
        });
