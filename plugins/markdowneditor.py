...
import plugins.markdowneditor_rc


class MarkdownEditor(ElementEditorInterface):
    close = Signal()

    def __init__(self):
        ElementEditorInterface.__init__(self)
        self.site = None
        self.class_name = "MarkdownEditor"
        self.display_name = "Markdown"
        self.tag_name = "Markdown"
        self.version = "1.0"
        self.icon = QImage(":/markdowneditor.png")
        self.changed = False
        ...
        self.titleLabel = QLabel(QCoreApplication.translate("MarkdownEditor", "Markdown Module"))
        fnt = self.titleLabel.font()
        fnt.setPointSize(16)
        fnt.setBold(True)
        self.titleLabel.setFont(fnt)

        boldButton = FlatButton(":/bold.svg", ":/bold.svg")
        italicButton = FlatButton(":/italic.svg", ":/italic.svg")
        linkButton = FlatButton(":/link.svg", ":/link.svg")
        imageButton = FlatButton(":/image.svg", ":/image.svg")
        tableButton = FlatButton(":/table.svg", ":/table.svg")
        ...
        toolbar.addWidget(tableButton)
        toolbar.addStretch()
        grid.addWidget(self.html, 2, 0, 1, 2)
        grid.addWidget(QLabel(QCoreApplication.translate("MarkdownEditor", "Admin Label")), 3, 0)
        grid.addWidget(self.adminlabel, 4, 0, 1, 2)
        self.setLayout(grid)

        close.clicked.connect(self.closeEditor)
        boldButton.clicked.connect(self.boldClicked)
        italicButton.clicked.connect(self.italicClicked)
        linkButton.clicked.connect(self.linkClicked)
        imageButton.clicked.connect(self.imageClicked)
        tableButton.clicked.connect(self.tableClicked)
    ...
    def boldClicked(self):
        ...
        self.html.setTextCursor(cursor)
        self.html.setFocus()
    ...
    def linkClicked(self):
        cursor = self.html.textCursor()
        pos = cursor.position()
        cursor.insertText("[Site](https://site.com)")
    ...
    def imageClicked(self):
        cursor = self.html.textCursor()
        pos = cursor.position()
        cursor.insertText("![Title](assets/images/imagename.png)")
        cursor.setPosition(pos)
        self.html.setTextCursor(cursor)
        self.html.setFocus()
    ...
    def registerContenType(self):
        qmlRegisterType(Markdown, 'MarkdownEditor', 1, 0, 'Markdown')

    def getImportString(self):
        return "import MarkdownEditor 1.0\n"
...
class Markdown(Item):
    def __init__(self, parent = None):
        ...
        self.tag_name = "Markdown"
        self.display_name = QCoreApplication.translate("MarkdownEditor", "Markdown")
    ...
    def getHtml(self):
        return markdown(html.unescape(self.text), html4tags = False, extras=["fenced-code-blocks", "wiki-tables", "tables", "header-ids"])
...
# this one is still a HTML syntax high lighter
